...
class ModelBase(object):
    ...
    def ask_target_iter(self, default_value=0):
        if io.get_default_answer('NonInteractiveMode') is not None:
                  default_target_iter = self.load_or_def_option('target_iter', default_value + int(io.get_default_answer('target_iter_increment')))
                  self.options['target_iter'] = max(0, default_target_iter + int(io.get_default_answer('target_iter_increment')))
