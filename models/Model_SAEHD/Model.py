...
""").lower()
    ...
    def on_initialize(self):
        ...
        if self.options['use_bn']:
            model_archi = nn.DeepFakeArchiBN(resolution, opts=archi_opts)
        else:
        ...
        with tf.device (models_opt_device):
            ...
            if self.is_training:
                ...
                lr=5e-7 # Changed, old value 5e-5
                ...
                if 'df' in archi_type:
                    ...
                    ### WEIGHTS FREEZING ###
                    ########################
                    # self.encoder.get_weights() + self.inter.get_weights() + self.decoder_dst.get_weights()
                    self.decoder_src.freeze_layers_by_name([
                            'upscalem0', 'upscalem1', 'upscalem2', 'upscalem3', 'out_convm'
                            ])
                    self.src_dst_trainable_weights = self.decoder_src.get_trainable_weights()
                elif 'liae' in archi_type:
        ...
        if self.is_training:
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/{devices[gpu_id].tf_dev_type}:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    if 'df' in archi_type:
                        # Added stop gradient
                        gpu_src_code     = tf.stop_gradient(self.inter(self.encoder(gpu_warped_src)))
                        gpu_dst_code     = tf.stop_gradient(self.inter(self.encoder(gpu_warped_dst)))
            ...
            def AE_view(warped_src, warped_dst):
                return nn.tf_sess.run ( [pred_src_src, pred_dst_dst, pred_src_srcm, pred_dst_dstm, pred_src_dst, pred_src_dstm],
                                            feed_dict={self.warped_src:warped_src,
                                                    self.warped_dst:warped_dst})
    ...
    def onTrainOneIter(self):
        ...
        # dst_loss
        src_loss = self.src_dst_train (warped_src, target_src, target_srcm, target_srcm_em, warped_dst, target_dst, target_dstm, target_dstm_em)
        ...
        for i in range(bs):
            ...
            self.last_dst_samples_loss.append (  (target_dst[i], target_dstm[i], target_dstm_em[i], 0 )  )
        ...
        if len(self.last_src_samples_loss) >= bs*16:
            ...
            # dst_loss
            src_loss = self.src_dst_train (target_src, target_src, target_srcm, target_srcm_em, target_dst, target_dst, target_dstm, target_dstm_em)
            self.last_src_samples_loss = []
            self.last_dst_samples_loss = []
    ...
    def onGetPreview(self, samples):
        ...
        S, D, SS, DD, SSM, DDM, SD, SDM = [ np.clip( nn.to_data_format(x,"NHWC", self.model_data_format), 0.0, 1.0) for x in ([target_src,target_dst] + self.AE_view (target_src, target_dst) ) ]
        ...
        if self.resolution <= 256:
            ...
            for i in range(n_samples // 2):
                ...
                st.append ( np.concatenate ( ar, axis=1) )
            if n_samples > 1:
                aux1 = np.concatenate(st, axis=0)
            st = []
            for i in range(n_samples // 2, n_samples):
                ar = S[i], SS[i]
                st.append ( np.concatenate ( ar, axis=1) )
            aux2 = np.concatenate(st, axis=0)
            if n_samples > 1:
                result += [ ('SAEHD src-src', np.concatenate ([aux1, aux2], axis=1 )), ]
            else:
                result += [ ('SAEHD src-src', aux2), ]
            ...
            aux2 = np.concatenate(st, axis=0)
            if n_samples > 1:
                result += [ ('SAEHD dst-dst', np.concatenate ([aux1, aux2], axis=1 )), ]
            else:
                result += [ ('SAEHD dst-dst', aux2), ]
            ...
            aux2 = np.concatenate(st, axis=0)
            if n_samples > 1:
                result += [ ('SAEHD pred', np.concatenate ([aux1, aux2], axis=1 )), ]
            else:
                result += [ ('SAEHD pred', aux2), ]
            ...
            for i in range(n_samples // 2):
                ...
                st_m.append ( np.concatenate ( ar, axis=1) )
            if n_samples > 1:
                aux1 = np.concatenate(st_m, axis=0)
            st_m = []
            for i in range(n_samples // 2, n_samples):
                ar = S[i]*target_srcm[i], SS[i]*SSM[i]
                st_m.append ( np.concatenate ( ar, axis=1) )
            aux2 = np.concatenate(st_m, axis=0)
            if n_samples > 1:
                result += [ ('SAEHD masked src-src', np.concatenate ([aux1, aux2], axis=1 )), ]
            else:
                result += [ ('SAEHD masked src-src', aux2), ]
            ...
            aux2 = np.concatenate(st_m, axis=0)
            if n_samples > 1:
                result += [ ('SAEHD masked dst-dst', np.concatenate ([aux1, aux2], axis=1 )), ]
            else:
                result += [ ('SAEHD masked dst-dst', aux2), ]
            ...
            aux2 = np.concatenate(st_m, axis=0)
            if n_samples > 1:
                result += [ ('SAEHD masked pred', np.concatenate ([aux1, aux2], axis=1 )), ]
            else:
                result += [ ('SAEHD masked pred', aux2), ]
